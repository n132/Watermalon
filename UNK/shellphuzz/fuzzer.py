...
class Fuzzer(object):
    ...
    ### EXPOSED 
    def start(self):# start fuzz with start afl + start timer 
        '''
        start fuzzing
        '''
    ...
    @property
    def alive(self):# check the count of all fuzzer (but I can't understand the os.kill..... )
    ...
    def kill(self):# terminate all the procs + stop timer + trun off the sign of Fuzzer._on
        for p in self.procs:
            p.terminate()
            p.wait()
    ...
    @property
    def stats(self):# Get the stats of Fuzzer  example: the content of workdir/shellphuzz/bug/bug/sync/fuzzer-master
        ...
        # stats exaple: {'foldername':dict of the info}
        # {'fuzzer-master': {'start_time': '1584516604', 'last_update': '1584516604', 'fuzzer_pid': '119356', 'cycles_done': '0', 'execs_done': '40', 'execs_per_sec': '563.38', 'paths_total': '5', 'paths_favored': '5', 'paths_found': '0', 'paths_imported': '0', 'max_depth': '4', 'cur_path': '4', 'pending_favs': '5', 'pending_total': '5', 'variable_paths': '0', 'stability': '100.00%', 'bitmap_cvg': '0.05%', 'unique_crashes': '0', 'unique_hangs': '0', 'last_path': '0', 'last_crash': '0', 'last_hang': '0', 'execs_since_crash': '40', 'exec_timeout': '20', 'afl_banner': 'fuzzer-master', 'afl_version': '2.52b', 'target_mode': 'qemu', 'command_line': '/home/n132/.local/lib/python3.7/site-packages/shellphish_afl/../bin/afl-unix/afl-fuzz -i - -o workdir/shellphuzz/bug/bug/sync -m 8G -Q -M fuzzer-master -x workdir/shellphuzz/bug/bug/bug.dict -- ./bug'}}

        return stats

    def found_crash(self):# return if get the crash 

        return len(self.crashes()) > 0

    def add_fuzzer(self):# add one fuzzer
        '''
        add one fuzzer
        '''
    ...
    def add_extension(self, name): # == Run `extensions.py ./binary `; stderr > log ; add new process to self.procs 
        """
    ...
    def add_fuzzers(self, n): # add n fuzzers
        for _ in range(n):
            self.add_fuzzer()

    def remove_fuzzer(self): # pop one and kill it 
        '''
        remove one fuzzer
        '''
    ...
    def remove_fuzzers(self, n): #pop n and kill all 
        '''
    ...
    def _get_crashing_inputs(self, signals): # get&list crashs 
        """
    ...
    def queue(self, fuzzer='fuzzer-master'):# get the list of all file in /fuzzer-master/queue
        '''
    ...
    def bitmap(self, fuzzer='fuzzer-master'): # get the data of /fuzzer-master/fuzz_bitmap
        '''
    ...
    def timed_out(self): # check if timed out 
    ...
    def _initialize_seeds(self):# set all seeds
        '''
    ...
    @staticmethod
    def _perform_env_checks():# I did not read the code ,maybe set the environ 
        err = ""
    ...
    @staticmethod
    def _get_base():#afl install directory
        '''
    ...
    def _export_library_path(self, p):#library path
        '''
    ...
    def _timer_callback(self):# trigger the callback when pending_favs==0 
